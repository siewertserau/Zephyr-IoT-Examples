...
// October 2011
//
// Adapted for use on Quark with Zephyr by Sam Siewert, April 2016
//
...
void  perform_move(int dim, int n, int k, double *X, int *cluster_assignment, double *cluster_centroid, int *cluster_member_count, int move_point, int move_target_cluster)
{
   ...
   // update cluster count array
    ...
    if (cluster_member_count[cluster_old] <= 1)
      PRINT("WARNING: Can't handle single-member clusters! \n");
...
void kmeans(
  ...
  {
    double distArray[n*k];
    int cluster_assignment_curArray[n]; 
    int cluster_assignment_prevArray[n];
    double point_move_scoreArray[n*k]; 

    double *dist  = (double *)distArray;
    int   *cluster_assignment_cur  = cluster_assignment_curArray;
    int   *cluster_assignment_prev = cluster_assignment_prevArray;
    double *point_move_score       = point_move_scoreArray;
   ...
   // BATCH UPDATE
    ...
    while (batch_iteration < MAX_ITERATIONS)
    {
          ...
          // failed to improve - currently solution worse than previous
           {
            ...
            // recalc centroids
             ...
             PRINT("  negative progress made on this step - iteration completed (%.2f) \n", totD - prev_totD);
        ...
        // move all points to nearest cluster
         ...
         PRINT("%3d   %u   %9d  %16.2f %17.2f\n", batch_iteration, 1, change_count, totD, totD - prev_totD);
         fflush(stdout);
         
        // done with this phase if nothing has changed
         if (change_count == 0)
           {
             PRINT("  no change made on this step - iteration completed \n");
             break;
           }
...
void main(void)
{
    ...
    PRINT("K-means test initialization\n");
    ...
    for(row=0; row<IMG_HEIGHT; row++)
    {
        for(col=0; col<IMG_WIDTH; col++)
        {
           ...
           PRINT("data point[%d,%d] = %u\n", row, col, grayVal[(row*IMG_WIDTH)+col]);
        }
    }

    PRINT("K-means centroid guesses\n");
    // guess at sky location
    cluster_centroid[0]=2.5;
    cluster_centroid[1]=10.0;
    // guess at water location
    cluster_centroid[2]=12.5;
    cluster_centroid[3]=10.0;

    PRINT("K-means test start\n");
